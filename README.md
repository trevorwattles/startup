...
- [x] **Backend listens for WebSocket connection** - In the backend, the WebSocket functionality is implemented in peerProxy.js. I used the “ws” library to create a WebSocket server that attaches directly to the existing HTTP server in index.js. This module listens for incoming WebSocket connections and marks each connection as alive with an isAlive property. It also establishes a keep-alive mechanism by sending periodic ping messages and handling pong responses. When a message is received from any client, the server broadcasts it to all other connected clients. This integration ensures that the backend can efficiently manage both HTTP and WebSocket traffic in real time.
- [x] **Frontend makes WebSocket connection** - On the client side, the WebSocket connection is established in jokeSocket.js. This file dynamically constructs the correct WebSocket URL based on the current protocol (using ws or wss accordingly) and creates a new WebSocket instance to connect to the backend. The connection lifecycle is managed with event handlers for onopen, onclose, and onmessage. These handlers provide real-time feedback on the connection status and automatically attempt reconnection if the connection drops. This robust implementation ensures that the frontend maintains a continuous, reliable connection with the backend for live updates.
- [x] **Data sent over WebSocket connection** - Data transmission over the WebSocket connection is handled through a structured JSON format. In jokeSocket.js, I defined a JokeMessage class that encapsulates the sender, event type, and payload. When the client needs to send data (for example, when saving a joke), it creates an instance of this class and serializes it to JSON. The backend, through the logic in peerProxy.js, receives these JSON messages and then broadcasts them to all connected clients, except for the sender. This design ensures that data is consistently formatted and reliably transmitted across the WebSocket connection.
- [x] **WebSocket data displayed** - The real-time data received from the WebSocket connection is integrated into the user interface through components like JokeGenerator.jsx and generate.jsx. These components register handlers on the client-side WebSocket instance, so when a new joke message arrives, the event is captured, parsed from JSON, and used to update the component’s state. This state change triggers a re-render, ensuring that the new data is immediately displayed in the UI. This dynamic updating mechanism provides users with instantaneous feedback and real-time joke updates without requiring a page refresh.
- [x] **Application is fully functional** - Overall, the application is fully functional, seamlessly integrating both HTTP and WebSocket services. The backend not only serves static content and API endpoints but also manages real-time communication through the WebSocket server. The frontend establishes a reliable WebSocket connection, handles the transmission and receipt of structured data, and updates the UI dynamically in response to real-time events. This cohesive integration results in an interactive and responsive application that meets all the deliverable requirements for live, real-time communication between the client and server.
